...
class Z3Model:
    ...
    clockValueFaultyPath = [Real("clock"+str(i+1)+"_fp_1")
                            for i in range(clockNum)]
    clockValueNormalPath = [Real("clock"+str(i+1)+"_np_1")
                            for i in range(clockNum)]
    ...
    sourceInvFaultyPath = [Bool("sourceInv"+str(i+1)+'_fp_1')
                           for i in range(clockNum)]
    sourceInvNormalPath = [Bool("sourceInv"+str(i+1)+'_np_1')
                           for i in range(clockNum)]
    finalInvFaultyPath = [Bool("finalInv"+str(i+1)+'_fp_1')
                          for i in range(clockNum)]
    finalInvNormalPath = [Bool("finalInv"+str(i+1)+'_np_1')
                          for i in range(clockNum)]

    lengthFaultyPath = [Int("length_fp_1")]
    lengthNormalPath = [Int("normal_np_1")]

    resetConstraintFaultyPath = [
        Int("reset" + str(i + 1) + "_fp_1") for i in range(clockNum)]
    resetConstraintNormalPath = [
        Int("reset" + str(i + 1) + "_np_1") for i in range(clockNum)]
